...
CYGWIN=binmode
export CYGWIN
export MIDINAME_SCRIPT_EDIT_DATE="2026-02-25"
...
export MIDINAME_LIBRARY="$MIDINAME-$MIDINAME_LIBRARY_API_VERSION"
...
if test "$DOVERSION" = "yes" ; then
   echo "Cfg66 version $MIDINAME_LIBRARY_VERSION $MIDINAME_SCRIPT_EDIT_DATE"
   exit 0
fi
...
if test "$DOHELP" = "yes" ; then
   ...
   cat << E_O_F
Usage: ./work [options]    ($MIDINAME_LIBRARY_VERSION-$MIDINAME_SCRIPT_EDIT_DATE)
...
be more to come. Some options might not work on Windows.
 ...
 --clean             Delete the usual derived files from the project. Also
                     do "git checkout doc/midiname-library-guide.pdf"
...
#  rm -f doc/latex/*.log
#  echo "Build products removed from the midiname/build directory."
#  git checkout doc/midiname-library-guide.pdf tests/data/fooout.rc
#  echo "Previous version of developer guide restored."
fi
...
# https://mesonbuild.com/Running-Meson.html
   ...
   echo "Making all midiname libraries..."
...
if test "$DOINSTALL" = "yes" ; then
   ...
   if test "$USERID" = 0 ; then
      cd build
      echo "Installing the midiname library..."
      meson install
      cd ..
   else
      echo "UID $USERID. We want you as root to install the midiname library..."
   fi
...
if test "$DOUNINSTALL" = "yes" ; then
   ...
   USERID=$(id -u)
   if test "$USERID" = 0 ; then
      echo "Uninstalling the midiname library..."
      ninja -C build uninstall
      if test "$PLATFORM" = "UNIX" ; then
         rm -rf "$INSTALL_PREFIX/include/$MIDINAME_LIBRARY"
         rm -rf "$INSTALL_PREFIX/$INSTALL_LIBDIR/$MIDINAME_LIBRARY"
         rm -rf "$INSTALL_PREFIX/share/doc/$MIDINAME"
#        rm -rf "$INSTALL_PREFIX/man/man1/$MIDINAME.1"
      fi
   else
      echo "UID $USERID. We want you as root to uninstall the midiname library..."
   fi
